...
#     data: The actual data, shape=(10918, 3), (m/s^2)
#     t: Timestamps in ms. every 0.005ms
#   imu_w: StampedData object with the imu rotational velocity (given in the vehicle frame).
#     data: The actual data, shape=(10918, 3), (rad/s)
#     t: Timestamps in ms. every 0.005ms
#   gnss: StampedData object with the GNSS data.
#     data: The actual data, shape=(55, 3)
#     t: Timestamps in ms. every 1.005ms
#   lidar: StampedData object with the LIDAR data (positions only).
#     data: The actual data, shape=(521,3)
#     t: Timestamps in ms. every 0.105ms
################################################################################################
gt = data['gt']
imu_f = data['imu_f']
imu_w = data['imu_w']
...
"""
c3 = np.array([[np.cos(0.1), -np.sin(0.1), 0],
               [np.sin(0.1), np.cos(0.1), 0],
               [0, 0, 1]])
c2 = np.array([[np.cos(0.05), 0, np.sin(0.05)],
               [0, 1, 0],
               [-np.sin(0.05), 0, np.cos(0.05)]])
c1 = np.array([[1, 0, 0],
               [0, np.cos(0.05), -np.sin(0.05)],
               [0, np.sin(0.05), np.cos(0.05)]])
c_li = c3.dot(c2).dot(c1)
"""
C_li = np.array([
...
p_cov[0] = np.zeros(9)  # covariance of estimated
gnss_i  = 0
lidar_i = 0
...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    I = np.eye(9)
    p_cov_hat = (I - K.dot(H)).dot(p_cov_check)
